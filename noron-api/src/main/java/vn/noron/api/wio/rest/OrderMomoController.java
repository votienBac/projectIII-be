...
public class OrderMomoController {
    ...
    @Operation(summary = "Danh sách order cua user")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Danh sách order cua user",
                    content = {@Content(
    ...
    @PostMapping("/order")
    public @NonNull Single<ResponseEntity<DfResponseList<OrderResponse>>> getOfUser(
            @PageableRequest Pageable pageable,
            Authentication authentication) {
        return orderMomoService.getOrderOfUser(AuthenticationUtils.loggedUserId(authentication), pageable)
                .map(users -> okEntity(users, pageable));
    }

    @Operation(summary = "Danh sách order")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Danh sách order",
                    content = {@Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = OrderResponse.class))}),
            @ApiResponse(responseCode = "404", description = "bad-request", content = @Content),
    ...
    public @NonNull Single<ResponseEntity<DfResponseList<OrderResponse>>> getAll(
            @PageableRequest Pageable pageable) {
        return orderMomoService.getOrder(pageable)
                .map(users -> okEntity(users, pageable));
    }
}
